...
def main(args):
    ...
    if args.type == "compile":
        true_labels = [test_dict["compilable_label"] for test_dict in df_test_dict]
    for idx, test_dict in enumerate(df_test_dict):
        ...
        if args.type == "sec":
            ...
            baseline_prompts.append(
                {
                    "prompt": build_prompt_function(prompt),
...
def eval_prompt(args):
    ...
    with open(f"{args.prompt_file}.txt", "w+") as f:
