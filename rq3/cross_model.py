...
def training_classify(train_df, test_df, field, lang="CPP", type="auto"):
    ...
    for i in range(5):
        if type == "auto":
            ...
            model = BinaryClassifierBase(embedding_dim)
...
def get_data_openia(model, par):
    ...
    df[f"emb"] = df["last_token_embedding"]
    df["label"] = df["label"].apply(lambda x: 1 if x else 0)
    with open("mbpp/standAlone_mbpp_test_task_id.txt") as f:
        test_ids = [int(l.strip()) for l in f.readlines()]
    df["partition"] = df["task_id"].apply(
...
for model in models:
    ...
    test_df = get_data_openia(model, "test")
    root_train_df = get_data_openia(model, "train")
    training_classify(root_train_df, test_df, f"openia_test", model, "base")
